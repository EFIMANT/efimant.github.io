...
document
  .querySelector(".closeW")
  ?.addEventListener("click", () => closeModalwhatsapp("pdfWhatsappModal"));
...
document
  ...
  ?.addEventListener("click", () => closeModalwhatsapp("empresaModal"));
...
document
  .getElementById("generatePdfWhatsappBtn")
    ...
    .then((data) => {
      if (data.status === "ok") {
        ...
        let mensaje = encodeURIComponent(
          " 🛠 *EFIMANT* 🛠 te ha generado una nueva cuenta de cobro:  " +
            fileURL +
            ", *_¡Estamos aquí para servirte!_*"
        );
...
function generatePDF(fecha, cuenta) {
  ...
  if (selectedEmpresa === "") {
    ...
    doc.text(
      `Ubicación:  ${empresaInfo.direccion || ""}, ${
        empresaInfo.ubicacion || ""
      }`,
      110,
      80
    );
  ...
  doc.setFontSize(10);
   ...
   // Línea para firmar (ajustada 10 puntos abajo)
  doc.line(30, startY + 9, 80, startY + 9);

  // Tamaño de fuente para el texto de la firma
  doc.setFontSize(8);

  // Texto "Firma y sello" también bajado 10 puntos
  doc.text("Firma", 52, startY + 12);
...
function generatePDFWhatsapp(fecha, cuenta) {
  ...
  // Añadir imagen de firma (bajada 10 puntos)
