...
function guardarDatos() {
    ...
    document.querySelectorAll("#tabla-body tr").forEach(fila => {
        ...
        datos.push({
            ...
            Empresa: celdas[5].textContent,
            total: celdas[6].textContent
        });
    });
...
function editarRegistro(boton) {
    ...
    document.getElementById("cantidad").value = limpiarNumero(celdas[1].textContent);
    document.getElementById("descripcion").value = celdas[2].textContent;
    document.getElementById("precio").value = limpiarNumero(celdas[3].textContent);
...
function obtenerTablaHTML() {
    ...
    filas.forEach(fila => {
        ...
        if (celdas.length > 6) { // Evita incluir la columna de botones
            tablaHTML += `<tr>
...
function enviarCorreo() {
    ...
    emailjs.send("service_isxerem", "template_fy80ibh", {
        to_name: "Jonathan Aldemar Estupiñan",
        from_name: "Efimant",
        message: tablaHTML
    }, "QbOQvaMnW1IMV30XA")
    .then(response => {
