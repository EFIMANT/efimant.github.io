const scriptURL = "https://script.google.com/macros/s/AKfycbwes5o6YzlvEXXuGb3H16sCGc7Y0_0boJ5IahPrtLYSU2vOMZagQI0E3baFJToOBk-6/exec";
...
function renderTable(data) {
    ...
    data.forEach((row, index) => {
        ...
        tableRow.insertCell(5).textContent = row.precio;
        tableRow.insertCell(6).textContent = row.total;
        tableRow.insertCell(7).textContent = row.empresa;

        const estadoCell = tableRow.insertCell(8);
        const estadoSelect = document.createElement("select");
        estadoSelect.classList.add("form-select");

        ["Pendiente", "Facturado", "Cancelado"].forEach(option => {
        ...
        const actionsCell = tableRow.insertCell(9);
        ...
        deleteButton.addEventListener("click", () => deleteRow(row, index, data));
...
function renderTable(data) {
    ...
    const actualizarColor = () => {
        estadoSelect.classList.remove("pendiente", "facturado", "cancelado");
    ...
    estadoCell.appendChild(estadoSelect);
        ...
        if (!isMobile) {
            ...
            deleteButton.classList.add("btn", "btn-danger");
            deleteButton.textContent = "Eliminar";
            deleteButton.addEventListener("click", () => showDeleteModal(tableRow, row, index, data));
            actionsCell.appendChild(deleteButton);
        } else {
            ...
            tableRow.addEventListener("touchmove", (e) => {
                ...
                if (deltaX > 0) {
                    ...
                    tableRow.style.backgroundColor = `rgba(255, 0, 0, ${Math.min(deltaX / 200, 0.7)})`;
                }
            });
            ...
            tableRow.addEventListener("touchend", (e) => {
                ...
                if (deltaX > 200) { // Umbral para eliminar
                    ...
                    tableRow.style.transition = "transform 0.8s ease-out, background-color 0.2s ease-out";
...
function showDeleteModal(tableRow, row, index, data) {
    ...
    newConfirmDelete.onclick = function () {
        ...
        tableRow.style.transition = "transform 0.3s ease-out, background-color 0.3s ease-out";
    ...
    closeButtons.forEach(button => {
        button.onclick = function () {
            modal.style.display = "none";
            tableRow.style.transition = "transform 0.2s ease-out, background-color 0.2s ease-out";
            tableRow.style.transform = "translateX(0)";
...
async function updateGoogleSheet(row, action) {
    ...
    try {
        ...
        if (responseText === "success" || responseText === "updated" || responseText === "deleted") {
            showCheckmark();
        } else {
        ...
        showErrorModal("Hubo un error al actualizar Google Sheets. Revisa la consola para más detalles.");
    }
}
...
document.getElementById("saveData").addEventListener("click", function () {
    ...
    if (!fecha || !placa || isNaN(cantidad) || cantidad <= 0 || !descripcion || isNaN(precio) || precio <= 0) {
    ...
    if (editRowIndex !== null) {
        cachedData[editRowIndex] = { fecha, placa, cantidad, descripcion, precio, total, estado, empresa, id: cachedData[editRowIndex].id };
        updateGoogleSheet(cachedData[editRowIndex], "update");
        editRowIndex = null;
    } else {
        const newRow = { id: "id_" + new Date().getTime(), fecha, placa, cantidad, descripcion, precio, total, estado, empresa };
    ...
    const modalElement = bootstrap.Modal.getInstance(document.getElementById("dataModal"));
