...
function enviarPorWhatsApp() {
    ...
    document.getElementById("sendToMe").onclick = function () {
        enviarPDFaWhatsApp("3115186410"); // Número predefinido
        modal.style.display = "none";
    };
...
function generatePDF(fecha, cuenta) {
    ...
    doc.setFontSize(9);
    doc.text("Duitama, Boyacá, Colombia", 15, 27);
    doc.setFont("helvetica", "bold");
    doc.text("Web:", 84, 17);
    doc.setFont("helvetica", "normal");
    doc.text(" efimant.com", 92, 17);
    doc.setFont("helvetica", "bold");
    doc.text("E-Mail:", 84, 22);
    doc.setFont("helvetica", "normal");
    doc.text(" efimant.oficial@gmail.com", 96, 22);
    doc.setFont("helvetica", "bold");
    doc.text("WhatsApp:", 84, 27);
    doc.setFont("helvetica", "normal");
    doc.text(" +57 318 814 7937", 101, 27);

    doc.text("Método de pago online: ", 150, 17);
    doc.text("Nequi", 150, 22);
    doc.setFontSize(10);
    // Línea separadora
    ...
    doc.text("Cra 34 #19-112 Duitama, Boyacá, Colombia", 20, 80);
    doc.text("Tel: 318 814 7937", 20, 85);
...
document.addEventListener("click", function (event) {
    if (event.target.matches("#descargarExcel") || event.target.matches("#descargarExcelModal")) {
        ...
        // Agregar encabezados
        let encabezados = [ "#", "Fecha","Placa", "Cantidad", "Descripción", "Precio", "Estado", "Empresa", "Total"];
        ...
        XLSX.writeFile(wb, "Registro_Clientes.xlsx");
    }
});
